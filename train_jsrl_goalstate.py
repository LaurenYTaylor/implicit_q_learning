        ...
        for seed in range(2):
            ...
            for i in tqdm.tqdm(steps, smoothing=0.1, disable=not FLAGS.tqdm):
                if i >= 1:
                    ...
                    dists.append(np.linalg.norm(np.array(env.target_goal) - np.array(env.get_xy())))
                    rel_dists.append(dists[-1] / max_dist)

                    if rel_dists[-1] >= horizons[horizon_idx]:
                ...
                if i % FLAGS.eval_interval == 0:
                    ...
                    eval_stats = evaluate_jsrl_goalstate(agent, env, FLAGS.eval_episodes, pretrained_agent, horizon, max_dist)
