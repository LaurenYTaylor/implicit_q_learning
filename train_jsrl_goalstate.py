...
from evaluation import evaluate, evaluate_jsrl_goalstate
    ...
    for i in tqdm.tqdm(steps, smoothing=0.1, disable=not FLAGS.tqdm):
        if i >= 1:
            if i == 1:
                ...
                max_dist = evaluate(pretrained_agent, env, 100)['goal_dist']
                horizons = np.arange(0, 1+1/FLAGS.curriculum_stages, 1/FLAGS.curriculum_stages)

            dist = np.linalg.norm(np.array(env.target_goal) - np.array(env.get_xy()))
            rel_dist = dist / max_dist

            if rel_dist >= horizons[horizon_idx]:
            ...
            if done:
                ...
                summary_writer.add_histogram(f'training/agent_type', np.array(agent_type), i, max_bins=2)
                agent_type = []
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            if i < 1:
                ...
                eval_stats = evaluate_jsrl_goalstate(agent, env, FLAGS.eval_episodes,
                                                     pretrained_agent, horizon, max_dist)
...
if __name__ == '__main__':
   #app.run(main)
   for d in [1000, 10000, 100000, 1000000]:
    main(0, d)
'''